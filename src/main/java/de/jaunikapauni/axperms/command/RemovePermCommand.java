...
public class RemovePermCommand implements CommandExecutor {
    AxPerms reference;
    public RemovePermCommand(AxPerms reference){
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String @NotNull [] args) {
        ...
        try(Connection conn = reference.getDatabaseManager().getConnection()){
            try(PreparedStatement ps = conn.prepareStatement("DELETE FROM perms WHERE uuid = ? AND permission = ?")){
                ps.setString(1, uuid.toString());
                ps.setString(2, permission);
                ps.executeUpdate();
                sender.sendMessage("Permission " + permission + " was deleted for " + targetName);
            }
